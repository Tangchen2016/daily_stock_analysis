    ...
    def get_stock_name(self, stock_code: str) -> Optional[str]:
        ...
        if _is_hk_code(code):
            try:
                ...
                if quote and quote.name:
                    logger.info(f"[股票名称] 港股 {code} -> {quote.name}")
                    return quote.name
            except Exception as e:
                logger.debug(f"[股票名称] 港股 {code} 获取失败: {e}")
        ...
        # A股：从实时行情获取名称
            ...
            try:
                quote = self.get_realtime_quote(code)
                if quote and quote.name:
                    logger.info(f"[股票名称] A股 {code} -> {quote.name}")
                    return quote.name
            except Exception as e:
                logger.debug(f"[股票名称] A股 {code} 获取失败: {e}")

        # 美股：暂不支持
        # akshare 没有提供美股名称查询的便捷接口
